...
class E_content(nn.Module):
  def __init__(self, input_dim_a, input_dim_b):
    ...
    n_cls = 10
    # CAM
    self.gap_fc = nn.Linear(tch, n_cls, bias = False)
    self.gmp_fc = nn.Linear(tch, n_cls, bias = False)

    self.conv1x1 = []
    self.conv1x1.append(nn.Conv2d(tch * 2, tch, kernel_size = 1, stride = 1, bias = True))
    self.conv1x1.append(nn.ReLU(inplace = True))
    self.conv1x1 = nn.Sequential(*self.conv1x1)

  def forward(self, xa, xb, label_A, label_B):
    ...
    # CAM
    gap__A = torch.nn.functional.adaptive_avg_pool2d(outputA, 1)
    gap__B = torch.nn.functional.adaptive_avg_pool2d(outputB, 1)

    gap_logit_A = self.gap_fc(gap__A.view(outputA.size(0), -1))
    gap_logit_B = self.gap_fc(gap__B.view(outputB.size(0), -1))

    gap_weight = list(self.gap_fc.parameters())[0]
    _, idx = torch.where(label_A == 1)
    gap_A = outputA * gap_weight[idx].unsqueeze(2).unsqueeze(3)
    _, idx = torch.where(label_B == 1)
    gap_B = outputB * gap_weight[idx].unsqueeze(2).unsqueeze(3)

    gmp__A = torch.nn.functional.adaptive_max_pool2d(outputA, 1)
    gmp__B = torch.nn.functional.adaptive_max_pool2d(outputB, 1) 

    gmp_logit_A = self.gmp_fc(gmp__A.view(outputA.size(0), -1))
    gmp_logit_B = self.gmp_fc(gmp__B.view(outputB.size(0), -1))

    gmp_weight = list(self.gmp_fc.parameters())[0]
    _, idx = torch.where(label_A == 1)
    gmp_A = outputA * gmp_weight[idx].unsqueeze(2).unsqueeze(3)
    _, idx = torch.where(label_B == 1)
    gmp_B = outputB * gmp_weight[idx].unsqueeze(2).unsqueeze(3)

    cam_logit_A = torch.cat([gap_logit_A, gmp_logit_A], 1)
    cam_logit_B = torch.cat([gap_logit_B, gmp_logit_B], 1)

    outputA = self.conv1x1(torch.cat([gap_A, gmp_A], 1))
    outputB = self.conv1x1(torch.cat([gap_B, gmp_B], 1))

    heatmap_A = torch.sum(outputA, dim = 1, keepdim = True)
    heatmap_B = torch.sum(outputB, dim = 1, keepdim = True)

    min_A = heatmap_A.min(dim = 2, keepdim = True)[0].min(dim = 3, keepdim = True)[0]
    min_B = heatmap_B.min(dim = 2, keepdim = True)[0].min(dim = 3, keepdim = True)[0]

    max_A = heatmap_A.max(dim = 2, keepdim = True)[0].max(dim = 3, keepdim = True)[0]
    max_B = heatmap_B.max(dim = 2, keepdim = True)[0].max(dim = 3, keepdim = True)[0]

    heatmap_A = (heatmap_A - min_A) / (max_A - min_A)
    heatmap_B = (heatmap_B - min_B) / (max_B - min_B)
    
    return outputA, cam_logit_A, heatmap_A, outputB, cam_logit_B, heatmap_B

  def forward_a(self, xa, label_A):
    outputA = self.convA(xa)
    outputA = self.conv_share(outputA)
...
class GaussianNoiseLayer(nn.Module):
  ...
  def forward(self, x):
    if self.training == False:
      return x
    noise = Variable(torch.randn(x.size()).cuda(x.get_device())) * 0.01
    return x + noise
