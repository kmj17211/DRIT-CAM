...
class DRIT(nn.Module):
  def __init__(self, opts):
    ...
    self.CAM = torch.nn.BCEWithLogitsLoss()
  ...
  def test_forward(self, image, label, a2b=True):
    self.z_random = self.get_z_random(image.size(0), self.nz, 'gauss')
    if a2b:
        self.z_content = self.enc_c.forward_a(image, label)
        output = self.gen.forward_b(self.z_content, self.z_random)
    else:
        self.z_content = self.enc_c.forward_b(image, label)
  ...
  def forward(self):
    ...
    # get encoded z_c
    self.z_content_a, self.cam_logit_A, self.heatmap_A, self.z_content_b, self.cam_logit_B, self.heatmap_B = self.enc_c.forward(self.real_A_encoded, self.real_B_encoded, self.label_A, self.label_B)
    ...
    # get reconstructed encoded z_c ################## reconstruction cam loss 추가해보기
    self.z_content_recon_b, _, _, self.z_content_recon_a, _, _ = self.enc_c.forward(self.fake_A_encoded, self.fake_B_encoded, self.label_B, self.label_A)
  ...
  def update_D_content(self, image_a, label_A, image_b, label_B):
    self.input_A = image_a
    self.label_A = label_A[:1]
    self.input_B = image_b
    self.label_B = label_B[:1]
  ...
  def update_D(self, image_a, label_A, image_b, label_B):
    self.input_A = image_a
    self.label_A = label_A[:1]
    self.input_B = image_b
    self.label_B = label_B[:1]
  ...
  def backward_EG(self):
    ...
    # CAM Loss
    loss_cam = self.CAM(self.cam_logit_A[:, :10], self.label_A) + self.CAM(self.cam_logit_B[:, :10], self.label_B) \
                + self.CAM(self.cam_logit_A[:, 10:], self.label_A) + self.CAM(self.cam_logit_B[:, 10:], self.label_B)
